...
astar
  ...
  -> (a -> Bool)       -- ^ isTarget
  -> a                 -- ^ source
  -> Maybe c -- ^ path from source to target
astar neighbors edgeCost heuristic isTarget source = go mempty (PQueue.singleton source 0 ())
 where
  go done queue =
    case PQueue.minView queue of
      Nothing -> Nothing
      Just (u, distU, (), queue') ->
        if isTarget u
          then Just distU
            ...
            go
              (S.insert u done)
              (foldr (step done u distU) queue' (neighbors u & filter (`S.notMember` done)))
  step done u distU v queue =
    let alt = distU + edgeCost u v
        distV = maybe maxBound fst (PQueue.lookup v queue)
     in if alt < distV
          then PQueue.insert v (alt + heuristic v) () queue
          else queue
