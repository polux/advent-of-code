...
neighbors spec@(_, _, dims) (S pos time) = wait ++ moves
 where
  ...
  moves = [S pos' time' | pos' <- n2d pos, free spec time' pos']
  n2d pos = [pos + d | d <- [V2 1 0, V2 (-1) 0, V2 0 1, V2 0 (-1)]]
  time' = time + 1

isSource :: State -> Bool
isSource (S (V2 x y) _) = x == 0 && y == -1

isTarget :: Spec -> State -> Bool
isTarget (_, _, V2 w h) (S (V2 x y) _) = x == w - 1 && y == h
...
solve input = time s3
 where
  spec = toSpec input
  s1 = last $ fromJust $ bfs (neighbors spec) (isTarget spec) initState
  s2 = last $ fromJust $ bfs (neighbors spec) isSource s1
  s3 = last $ fromJust $ bfs (neighbors spec) (isTarget spec) s2
