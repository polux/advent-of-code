...
--solve :: Input -> Output
solve input = length [p | x <- [minX..maxX], let p = V2 x row, cannotBeBeacon p]
  where
    minX = traceShowId $ minimum [s^._x-d | (s,d) <- sensors]
    maxX = traceShowId $ maximum [s^._x+d | (s,d) <- sensors]
    sensors = [(s, distance s b) | (s,b) <- input]
    beacons = S.fromList (map snd input)
    cannotBeBeacon p = or [distance p s <= d | (s,d) <- sensors] && p `S.notMember` beacons
