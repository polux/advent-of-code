...
--solve :: Input -> Output
solve input = bisect ((>=0) . diff) (approxRoot-1_000) (approxRoot+1_000)
  where
    approxRoot = round (- fromIntegral (diff 0) / slope)
    slope = fromIntegral (diff 1_000 - diff 0) / 1_000
    BinOp _ lhs rhs = input M.! "root"
    diff :: Int -> Int
    diff humn = meval lhs - meval rhs
      where
        meval = memo eval
        eval "humn" = humn
        eval x =
