...
startingGrid :: Grid
startingGrid = M.fromList (zip [V2 x y | x <- [2, 4, 6, 8], y <- [1, 2]] [D,B,D,A,C,A,B,C])

winningGrid :: Grid
winningGrid = M.fromList [(V2 (room c) y, c) | c <- [A, B, C, D], y <- [1, 2]]

availableRoomSpot :: Grid -> Letter -> Maybe (V2 Int)
availableRoomSpot grid c =
  case (grid M.!? pos1, grid M.!? pos2) of
    (Nothing, Nothing) -> Just pos2
    (Nothing, Just d) | d == c -> Just pos1
    _ -> Nothing
 where
  pos1 = V2 (room c) 1
  pos2 = V2 (room c) 2
...
isRoomDone grid c = (grid M.!? V2 (room c) 1, grid M.!? V2 (room c) 2) == (Just c, Just c)
...
path (V2 x1 y1) (V2 x2 0) = [V2 x1 1 | y1 == 2] ++ [V2 x 0 | x <- range x1 x2]
