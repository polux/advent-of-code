...
-- >>> let { f 1 = [2,3]; f 2 = [4]; f 3 = [5, 6]; f 5 = [4]; f _ = []} in bfs f 1 (==4)
-- Just [1,2,4]
bfs
  :: Ord a
  => (a -> [a])  -- ^ neighbors
...
bfs neighbors source isTarget = go mempty (Seq.singleton [source])
 where
  go _ Empty = Nothing
...
dijkstra
  :: (Ord a, Num c, Ord c, Bounded c, Show c, Show a)
  => (a -> [a])        -- ^ neighbors
  -> (a -> a -> c)     -- ^ cost function
  -> (a -> Bool)       -- ^ isTarget
  -> a                 -- ^ source
 ...
 where
   go done queue =
     case PQueue.minView queue of
       Nothing -> Nothing
       Just (u, cost, path, queue') ->
         if isTarget u
           then Just (cost, reverse path)
           else go (S.insert u done) (foldr (step done u path cost) queue' (neighbors u & filter (`S.notMember` done)))
