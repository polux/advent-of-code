...
game :: IO ()
game = play FullScreen black 0 (V2 1 0) display update (const id)
 where
  display v = pictures [speedVec, targetRec, trajectory, maxYText, velText]
   where
...
vxLowerBound = 0 --19
vxUpperBound = target ^. _2 . _x
vyLowerBound = target ^. _1 . _y
vyUpperBound = 93
...
goodVelocity v = any inside (take 1000 (map fst (trace v)))
  ...
  print $
    length $
      filter
        goodVelocity
        [ V2 vx vy
        | vx <- [vxLowerBound .. vxUpperBound]
        , vy <- [vyLowerBound .. vyUpperBound]
        ]