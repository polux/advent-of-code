...
import Optics (AffineTraversal', Prism', Traversal', at, each, filtered, filteredBy, folded, has, isn't, ix, lensVL, only, preuse, sans, sumOf, to, traversed, use, view, (%), (%~), (&), (.~), (?~), (^.), (^..), (^?), _1, _2, _3, _4)
import Optics.Label ()
...
attack :: Grid -> V2 Int -> Grid
attack grid target =
  if grid ^?! creatureHpAt target <= 3
    then grid & ix target .~ Floor
    else grid & creatureHpAt target %~ (\hp -> hp -3)
...
playGame :: Int -> State Grid Int
playGame i = do
  grid <- get
  if length (S.fromList (grid ^.. folded % #_Creature % #cType)) < 2
    then return i
    else do
      res <- runExceptT playRound
      case res of
        Left () -> return i
        Right () -> playGame (i + 1)


-- On my input, and only on my input, the total hit point sum is 3 points too high
solve input = (numRounds, sumOf (folded % #_Creature % #cHp) finalGrid)
  where
    initialGrid = toGrid input
    (numRounds, finalGrid) = runState (playGame 0) initialGrid
