...
initialCarts :: Input -> Carts
initialCarts = M.fromList . zip [0..] . mapMaybe toCart . UA.assocs
 where
...
ticks :: Input -> Carts -> [Carts]
ticks input carts = go carts (sortOn (pos.snd) (M.toList carts) & map fst)
  where
    go lastCarts [] = lastCarts : ticks input lastCarts
    go lastCarts (id:ids)
      | Just cart <- lastCarts M.!? id =
          let cart' = step input cart
              collisions = [oid | (oid, ocart) <- M.toList lastCarts, oid /= id, pos cart' == pos ocart]
              newCarts = if null collisions then lastCarts & at id ?~ cart' else foldr M.delete lastCarts (id:collisions)
          in go newCarts ids
      | otherwise = go lastCarts ids
