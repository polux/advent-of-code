...
The program stops when r5==r0 on line 28, so we just need to print the value of
r5 when line 28 is executed the first time.
...
solve :: Input -> Int
solve (ip, prog) = go (UV.fromList [1, 0, 0, 0, 0, 0])
  where
    go mem =
      let n = mem UV.! ip
       in if n == 28
            then mem UV.! 5
            else case prog V.!? n of
              Nothing -> error "reached end of program"
              Just (op, a, b, c) ->
