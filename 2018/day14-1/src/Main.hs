...
solve :: Int -> String
solve input = go (Seq.fromList [3, 7]) 0 1 & toList & concatMap show
 where
   go :: Seq Int -> Int -> Int -> Seq Int
   go seq i j
     | length seq >= input+10 = Seq.take 10 (Seq.drop input seq)
     | otherwise =
