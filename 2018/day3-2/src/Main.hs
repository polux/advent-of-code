...
overlap :: (V2 Int, V2 Int) -> (V2 Int, V2 Int) -> Bool
overlap (V2 x1 y1, V2 w1 h1) (V2 x2 y2, V2 w2 h2) = x2 < x1+w1 && x1 < x2+w2 && y2 < y1+h1 && y1 < y2+h2

solve :: Input -> Output
solve input =
  input
  & M.assocs
  & filter (\(id, rect) -> not $ any (overlap rect) (M.elems (M.delete id input)))
  & map fst
  & head
