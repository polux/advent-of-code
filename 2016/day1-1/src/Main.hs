...
data State = State { dir :: V2 Int, pos :: V2 Int }

solve :: Input -> Output
solve input =
  let V2 x y = pos $ foldl' next (State (V2 0 1) (V2 0 0)) input
  in abs x + abs y
 where
  next (State dir pos) (turn, n) =
    let newDir = rotate turn dir
    in State newDir (pos + n *^ newDir)
