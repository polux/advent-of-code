...
solve = foldl' step initStr . reverse
 where
   ...
   step str (Move i j) =
     let ci = str `Seq.index` j
     in Seq.insertAt i ci (Seq.deleteAt j str)
   step str (RotateBasedOn a) =
     head (filter ((== str).f) [step str (RotateRight i) | i <- [1..length str]])
      where
         f cand =
           let ai = fromJust $ Seq.elemIndexL a cand
           in step cand (RotateLeft (1 + ai + if ai >= 4 then 1 else 0))
