...
-- found by iterating dance: dance (dance (dance initState)) == dance (dance initState)
--solve :: [Instr] -> Int
solve input =
  let Just period = (+1) <$> elemIndex initState (tail states)
      (offset, UV.toList->str) = states !! (1_000_000_000 `mod` period)
      n = (len-offset)
  in drop n str ++ take n str
 where
   states = iterate dance initState
   initState = (0, UV.fromList chars)
   dance state = foldl' step state input & _1 %~ (`mod` len)
   chars = ['a'..'p']
   len = length chars
