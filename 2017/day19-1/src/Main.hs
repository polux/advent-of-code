...
step :: Input -> St -> Writer [Char] (Maybe St)
step grid (St pos dir) =
  case at pos of
     '|' -> return $ Just $ St (pos+dir) dir
     '-' -> return $ Just $ St (pos+dir) dir
     '+' ->
       let ldir = left dir
           rdir = right dir
           lpos = pos + ldir
           rpos = pos + rdir
       in return $ Just (if at lpos == ' ' then St rpos rdir else St lpos ldir)
     ' ' -> return Nothing
     c -> do
       tell [c]
       return $ Just $ St (pos+dir) dir
...
solve :: Input -> Output
solve input = execWriter (go (St (findStartingPoint input) (V2 0 1)))
 where
   go st = step input st >>= mapM_ go