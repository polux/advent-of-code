...
solve :: Input -> Output
solve input = maximum (map score (filter ((==500) . calories) candidates))
 where
   candidates = sumsTo (length input) 100
   factors = init (transpose input) -- ignore calories
   caloriesFactors = last (transpose input)
   calories candidate = properties candidate caloriesFactors
