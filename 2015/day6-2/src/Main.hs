...
on :: Grid -> Coord -> Grid
on grid coord = M.insertWith (+) coord 1 grid

off :: Grid -> Coord -> Grid
off grid coord = M.insertWith (\_ old -> max 0 (old-1)) coord 0 grid

toggle :: Grid -> Coord -> Grid
toggle grid coord = M.insertWith (+) coord 2 grid
...
solve :: Input -> Output
solve input = foldl' execute mempty input & sum
