...
run (name, speed, runLen, restLen) = scanl' (+) 0 (fly runLen)
 where
   fly 0 = rest restLen
   fly n = speed : fly (n-1)
   rest 0 = fly runLen
...
distanceAfter :: Int -> (String, Int, Int, Int) -> Int
distanceAfter n reinder = run reinder !! n

solve :: Input -> Output
solve input = maximum (map (distanceAfter 2503) input)
