...
dijkstra neighbors edgeCost isTarget source = go mempty (PQueue.singleton source 0 [source])
 where
  go done queue =
...
astar
  ...
  -> a                 -- ^ source
  -> Maybe [a] -- ^ path from source to target
astar neighbors edgeCost isTarget heuristic source = go mempty (PQueue.singleton source 0 [source])
 where
  go done queue =
    case PQueue.minView queue of
      Nothing -> Nothing
  ...
  step done u pathToU distU v queue =
    let alt = distU + edgeCost u v
        distV = maybe maxBound fst (PQueue.lookup v queue)
     in if alt < distV
          then PQueue.insert v (alt + heuristic v) (v : pathToU) queue
          else queue
...
instance HasTrie a => HasTrie (V2 a) where
  newtype (V2 a :->: b) = V2Trie { unV2Trie :: (a,a) :->: b }
  trie f = V2Trie (trie (\(i, j) -> f (V2 i j)))
  untrie (V2Trie t) (V2 i j) = untrie t (i, j)
  enumerate (V2Trie t) = [(V2 i j, x) | ((i,j),x) <- enumerate t]