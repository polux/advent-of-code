...
parse = map parseLine . lines
 where
  parseLine str =
    let ws = words str
    in (parseHead (head ws), map parseButton (init (tail ws)), parseLast (last ws))
  parseHead (init.tail->s) = fromBinary [if b == '.' then 0 else 1 | b <- reverse s]
  parseButton s = foldr (.|.) 0 [1 `shiftL` n | n <- ints s]
  parseLast = ints
...
solve input = map solveLine input & sum
  where
    solveLine (target, buttons, _) = bfs (\state -> [state `xor` b | b <- buttons]) (==target) 0 & fromJust & length & pred
