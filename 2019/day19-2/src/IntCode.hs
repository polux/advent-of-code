...
module IntCode (Machine(), parseAndExecute, parseAndExecuteIO, toAscii, evalList, parseToMachine, evalIO) where
...
data Run :: * -> * where
  Done :: a -> Run a
  ReadInput :: (Int -> Run a) -> Run a
  WriteOutput :: Int -> Run a -> Run a
...
parseToMachine :: String -> Machine
parseToMachine = run . parse
