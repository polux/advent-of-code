...
paint vec = grid $ flip execState initialState $ unRunRobot $ run 0 0 (vec <> V.replicate 10000 0)
 where
  initialState = World { position = (0,0), direction = N, grid = M.singleton (0,0) 1, robotState = WaitingColor }

render :: M.Map Position Integer -> String
render grid = unlines [[toPixel (M.findWithDefault 0 (x,y) grid) | x <- [0..maxX]] | y <- [0..maxY]]
 where
  whites = M.keys $ M.filter (==1) grid
  maxX = maximum (map fst whites)
  maxY = maximum (map snd whites)
  toPixel 0 = '█'
  toPixel 1 = ' '
