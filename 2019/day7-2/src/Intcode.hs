...
executePermutation prog [i0,i1,i2,i3,i4] = last res4
 where
  res0 = execute (i0:0:res4) prog
  res1 = execute (i1:res0) prog
  res2 = execute (i2:res1) prog
  res3 = execute (i3:res2) prog
  res4 = execute (i4:res3) prog

bestOutput :: Vec -> Int
bestOutput prog = maximum (map (executePermutation prog) (permutations [5..9]))
