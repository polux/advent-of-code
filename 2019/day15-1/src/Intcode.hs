...
instance Show a => Show (Run a) where
  ...
  show (ReadInput k) = "ReadInput <k>"
  show (WriteOutput n k) = "WriteOutput " ++ show n ++ " <k>"
...
shortestPathLength :: Vec -> Int
shortestPathLength vec = minimum (explore S.empty (0,0) 1 machine)
 where
...
explore :: S.Set (Int, Int) -> (Int, Int) -> Int -> Machine -> [Int]
explore seen pos len (ReadInput k)= do
  direction <- [1,2,3,4]
  let newPos = move pos direction
  guard (not (S.member newPos seen))
  let (WriteOutput status m) = k direction
  case status of
    0 -> pure maxBound
    1 -> explore (S.insert newPos seen) newPos (succ len) m
    2 -> pure len
    _ -> error "unexpected return code"
explore _ _ _ _ = error "unexpected machine state"
