...
data Instr = NewStack | Cut Int | Increment Int
  deriving Show
type Input = [Instr]
...
apply :: Input -> Int -> Int
apply input i = foldl' (flip step) i input

step :: Instr -> Int -> Int
step NewStack i = deckSize-1-i
step (Cut j) i = (i - j) `mod` deckSize
step (Increment n) i = (i*n) `mod` deckSize

solve :: Input -> Output
solve input = apply input 2019
