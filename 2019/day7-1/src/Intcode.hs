...
executePermutation :: Vec -> [Int] -> Int
executePermutation prog [i0,i1,i2,i3,i4] = head res4
 where
  res0 = execute [i0, 0] prog
  res1 = execute [i1, head res0] prog
  res2 = execute [i2, head res1] prog
  res3 = execute [i3, head res2] prog
  res4 = execute [i4, head res3] prog

bestOutput :: Vec -> Int
bestOutput prog = maximum (map (executePermutation prog) (permutations [0..4]))
