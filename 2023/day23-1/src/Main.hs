 ...
 where
  ...
  go seen prev pos
    ...
    | otherwise =
        maximumDef
          (minBound @Int)
          [1 + go (S.insert pos seen) pos n | n <- neighbors prev pos, n `S.notMember` seen]