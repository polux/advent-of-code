...
-- solve :: Input -> Output
solve input = sum [length (chain n) - 1 | (n, _) <- named]
 where
  ...
  supportedBy n = fromMaybe [] (n `M.lookup` supports)

  chain n = go (S.singleton n) (Seq.fromList (supportedBy n))
   where
    go fallen Empty = fallen
    go fallen (n :<| ns)
      | (sitsOn M.! n) `S.isSubsetOf` fallen = go (S.insert n fallen) (ns >< Seq.fromList (supportedBy n))
      | otherwise = go fallen ns
