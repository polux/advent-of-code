...
--solve :: Input -> Output
solve input = mydijkstra neighbors edgeCost isTarget sources & fmap fst -- fromJust & snd & map (\(cell,_,_) -> input UA.! cell)
 where
  sources :: [Node]
  sources = [(V2 0 0, V2 1 0, 0), (V2 0 0, V2 0 1, 0)]
  (V2 w h) = arraySize input
  isTarget (cell,_,steps) = cell == V2 (w-1) (h-1) && steps >= 4

  inside (V2 x y) = x >= 0 && x < w && y >= 0 && y < h

  neighbors (pos, dir, steps) = do
    delta <- if steps < 4 then [dir] else [V2 0 1, V2 0 (-1), V2 1 0, V2 (-1) 0]
    guard (delta /= -dir)
...
mydijkstra neighbors edgeCost isTarget sources = go mempty (PQueue.fromList [(v,0,[v]) | v <- sources])
 where
  --go d q | traceShow (d, PQueue.toList q) False = undefined
  go done queue =
