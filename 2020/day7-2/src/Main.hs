...
parse str = toGraph $ runParserOrDie (inputParser <* P.eof) str
  where
    ...
    entryParser = do
      ...
      bagOrBags
      containOrContains
      bags <- (noOtherBags $> []) P.<|> (bagParser `P.sepBy` symbol ",")
      return (color, bags)
    bagParser = do
      ...
      return (color, num)
    colorParser = do
      word1 <- lexeme (P.some P.alphaNumChar)
      word2 <- lexeme (P.some P.alphaNumChar)
      return (word1, word2)
    bagOrBags = lexeme (P.string "bag" <* P.optional (P.char 's'))
    containOrContains = lexeme (P.string "contain" <* P.optional (P.char 's'))
    noOtherBags = symbol "no other bags"

solve :: Input -> Output
solve (graph, namedNodes) = go graph (namedNodes M.! ("shiny", "gold")) - 1
 where
   go :: Gr Color Int -> Int -> Int
   go gr n = 1 + sum [num * go gr i | (i, _, num) <- G.inn gr n]